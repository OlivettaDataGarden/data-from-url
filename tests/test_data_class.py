...
def test_query_params_headers_is_dict():
    ...
    """
    assert isinstance(QueryParams(**DEFAULT_QUERY_PARAMS).headers, dict)
...
def test_query_params_default_method_is_get():
    ...
    """
    assert QueryParams(**DEFAULT_QUERY_PARAMS).method == RestMethod.GET.value
...
def test_query_params_method_can_be_set_to_post():
    ...
    assert QueryParams(**query_params).method == RestMethod.POST.value
...
def test_query_params_with_invalid_method():
    ...
    with pytest.raises(ValueError) as validation_error:
        QueryParams(**query_params)
    assert GetDataExceptions.INVALID_METHOD.value in str(validation_error)
