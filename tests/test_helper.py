...
from imports import CONNECTIVITY_ERROR_RESPONSE, GetDataResponse, \
    convert_params_to_url_ext, helper, make_request_with_method, \
    normalize_string, requests_retry_session, retry_after_connection_error
...
def test_retry_after_connection_error_with_connection_error_1():
    ...
    """
    @retry_after_connection_error(
            retries=3, waittime=0,
            return_value_on_fail=CONNECTIVITY_ERROR_RESPONSE)
    def req_method(*args, **kwargs):
...
def test_requests_retry_session():
    ...
    assert isinstance(
        requests_retry_session(**session_params), requests.Session)
...
def test_requests_retry_session_updates_retry_values():
    ...
    updated_session = requests_retry_session(
        session=session, retries=3, backoff_factor=0.3,
