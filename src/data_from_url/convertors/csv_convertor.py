...
class CSVConvertor(AbstractConvertor):
    ...
    def _result_with_data_field(cls, result, convertor_params):
        ...
        list_of_dicts = cls._zip_header_and_reader(
            csv_reader, header, convertor_params)

        if list_of_dicts:
            return cls._valid_result_from_csv(
                result, {'result': list_of_dicts})
    ...
    def _valid_result_from_csv(
            result: GetDataResponse, valid_csv_data: dict) -> GetDataResponse:
        """ return class return object for succesfull cvs conversion """
        return GetDataResponse(
            response=result.response, is_valid=True, data=valid_csv_data,
    ...
    def _validate_convertor_params(convertor_params=None) -> None:
        ...
        if isinstance(convertor_params, dict) and \
                convertor_params.get('delimiter', None):
            return
