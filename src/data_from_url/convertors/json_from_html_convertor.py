...
from .json_from_html_helper import find_str_sequence_in_str, \
    get_error_content_from_json_exception, get_html_from_response, \
    get_string_between_quotes, replace_strings
...
class JSONFromHTMLConvertor(AbstractConvertor):
    ...
    def _result_with_data_field(cls, result, convertor_params):
        ...
        json_byte_in_html = cls._find_json_byte_in_html(
            html_in_byte_resp=html_in_byte_repr,
    ...
    def _invalid_json_result(
        ...
        return GetDataResponse(
            response=result.response, is_valid=False, data=None,
            error_msg=error_msg, io_time=result.io_time)
    ...
    def _valid_result_with_json(
                result: GetDataResponse, valid_json: dict) -> GetDataResponse:
        """ return class return object for succesfull json conversion """
        return GetDataResponse(
            response=result.response, is_valid=True, data=valid_json,
            error_msg=result.error_msg, io_time=result.io_time)
    ...
    def _validate_convertor_params(convertor_params=None) -> None:
        ...
        if not convertor_params:
            raise ValueError(
                GetDataExceptions.NO_RESPONSE_CONVERTOR_PARAMS.value)

        if isinstance(convertor_params, dict) and \
                all([key in CONVERTOR_PARAMS_JSON_FROM_HTTP for
                    key in convertor_params.keys()]) and \
                'json_begin' in convertor_params.keys() and \
                'json_end' in convertor_params.keys():
            return

        raise ValueError(
            GetDataExceptions.INVALID_RESPONSE_CONVERTOR_PARAMS.value)
    ...
    def _find_json_byte_in_html(
        ...
        if convertor_params.get('json_within_quotes', None):
            byte_string_containing_json = \
                html_in_byte_resp[begin:end + len(json_end)]
            return get_string_between_quotes(
                byte_string_containing_json
            )
        ...
        return byte_string_containing_json[
            len(json_begin):
        ].strip()
    ...
    def _format_json(
            ...
            result: GetDataResponse,
            convertor_params: dict,
            json_in_byte: bytes) -> GetDataResponse:
        """
        """
